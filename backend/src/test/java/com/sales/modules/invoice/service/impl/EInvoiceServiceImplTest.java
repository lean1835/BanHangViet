...
import com.sales.modules.invoice.repository.EInvoiceItemRepository;
...
class EInvoiceServiceImplTest {
    ...
    void testCreateInvoiceDraft_withoutPointRedemption_returnsZeroPointsAndCorrectTax() {
        ...
        com.sales.modules.order.entity.Order order = com.sales.modules.order.entity.Order.builder()
                ...
                .paymentStatus("DEBT")
                .paymentMethod("DEBT")
                .totalAmount(new BigDecimal("110000.00"))
                .discountAmount(new BigDecimal("30000.00"))
                .pointDiscountAmount(BigDecimal.ZERO)
                .pointsRedeemed(0)
                .taxAmount(new BigDecimal("7000.00"))
                .finalAmount(new BigDecimal("77000.00"))
                .items(new ArrayList<>())
                .build();
        ...
        when(userRepository.findByUsername("seller1")).thenReturn(Optional.of(currentUser));
        when(orderRepository.findByIdAndHouseholdIdAndDeletedAtIsNull("ord-no-points", "hh-100"))
                .thenReturn(Optional.of(order));
        when(eInvoiceRepository.findByOrderIdAndDeletedAtIsNull("ord-no-points"))
                .thenReturn(Optional.empty());
        when(invoiceTemplateRepository.findByHouseholdId("hh-100"))
                .thenReturn(Optional.empty());
        when(invoiceTemplateRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
        when(eInvoiceRepository.save(any(EInvoice.class))).thenAnswer(inv -> {
            EInvoice invObj = inv.getArgument(0);
            invObj.setId("inv-no-points");
            return invObj;
        });

        InvoiceResponse response = eInvoiceService.createInvoiceDraft("seller1", "ord-no-points");

        assertNotNull(response);
        assertEquals(new BigDecimal("70000.00"), response.getTotalAmountBeforeTax());
        assertEquals(new BigDecimal("7000.00"), response.getTaxAmount());
        assertEquals(new BigDecimal("30000.00"), response.getDiscountAmount());
        assertEquals(BigDecimal.ZERO, response.getPointDiscountAmount());
