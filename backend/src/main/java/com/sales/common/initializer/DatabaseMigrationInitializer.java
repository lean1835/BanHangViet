package com.sales.common.initializer;
...
public class DatabaseMigrationInitializer implements CommandLineRunner {
    ...
    private boolean isMySQL() {
        try {
            if (jdbcTemplate.getDataSource() == null) return false;
            try (var conn = jdbcTemplate.getDataSource().getConnection()) {
                String prod = conn.getMetaData().getDatabaseProductName();
                return prod != null && prod.toLowerCase().contains("mysql");
            }
        } catch (Exception e) {
            return false;
        }
    ...
    public void run(String... args) {
        if (isMySQL()) {
            try {
                ...
                log.warn("DatabaseMigrationInitializer: Bỏ qua kiểm tra trigger database: {}", e.getMessage());
            }
