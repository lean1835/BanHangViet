...
public class JwtFilter extends OncePerRequestFilter {
    ...
    private final com.sales.modules.auth.service.UserSessionService userSessionService;
    private final com.sales.modules.auth.repository.UserRepository userRepository;
    private final com.sales.modules.auth.repository.BusinessHouseholdRepository businessHouseholdRepository;
    private final com.sales.modules.tax.repository.HouseholdAccountantAssignmentRepository assignmentRepository;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
                ...
                userRepository.findByUsername(authUsername).ifPresent(user -> {
                    if (user.getRole() != null && "VT-03".equals(user.getRole().getCode())) {
                        boolean hasAssignment = assignmentRepository.existsByHouseholdIdAndAccountantUserIdAndStatus(
                                householdContextId.trim(), user.getId(), com.sales.common.constant.AccountantAssignmentStatus.ACTIVE);
                        if (hasAssignment) {
                            businessHouseholdRepository.findById(householdContextId.trim())
                                    .ifPresent(com.sales.common.security.HouseholdContextHolder::setHousehold);
                        }
                    }
                });
            }
            ...
            com.sales.common.security.HouseholdContextHolder.clear();
        }
    }
}
