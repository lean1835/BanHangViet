...
public interface TaxConnectionLogRepository extends JpaRepository<TaxConnectionLog, String> {
    ...
    @Query("SELECT l FROM TaxConnectionLog l WHERE (l.household.id = :householdId OR l.household IS NULL) AND l.createdAt >= :fromDate ORDER BY l.createdAt DESC")
    List<TaxConnectionLog> findLogsByHouseholdAndDateAfter(@Param("householdId") String householdId,
                                                          @Param("fromDate") LocalDateTime fromDate);

    @Query("SELECT l FROM TaxConnectionLog l WHERE (l.household.id = :householdId OR l.household IS NULL) ORDER BY l.createdAt DESC")
