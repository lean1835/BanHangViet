...
import com.sales.modules.customer.dto.response.ImportCustomerResultResponse.RowErrorDetail;
    ...
    public ImportCustomerResultResponse importCustomers(String currentUsername, MultipartFile file, String duplicateAction) {
        ...
        try (Workbook workbook = WorkbookFactory.create(file.getInputStream())) {
            ...
            if (!customersToCreate.isEmpty()) {
                List<Customer> allSavedCustomers = new ArrayList<>(customersToCreate.size());
                for (int i = 0; i < customersToCreate.size(); i += 500) {
                    List<Customer> batch = customersToCreate.subList(i, Math.min(i + 500, customersToCreate.size()));
                    allSavedCustomers.addAll(customerRepository.saveAll(batch));
                }

                List<CustomerDebt> debtsToSave = new ArrayList<>();
                for (int i = 0; i < allSavedCustomers.size(); i++) {
                    Customer savedCustomer = allSavedCustomers.get(i);
                ...
                if (!debtsToSave.isEmpty()) {
                    for (int i = 0; i < debtsToSave.size(); i += 500) {
                        List<CustomerDebt> batch = debtsToSave.subList(i, Math.min(i + 500, debtsToSave.size()));
                        customerDebtRepository.saveAll(batch);
                    }
                }
            }
        } catch (Exception e) {
