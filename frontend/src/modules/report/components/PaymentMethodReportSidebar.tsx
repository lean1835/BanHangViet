...
export const PaymentMethodReportSidebar: React.FC = () => {
  const { paymentMethodFilter, setPaymentMethodFilter, setPaymentMethodPreset } = useReportFilter();
    ...
    <div className="flex flex-col gap-3.5 text-xs animate-in fade-in duration-200">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="grid grid-cols-3 gap-1 text-[10px] font-bold">
          ...
          <button
            type="button"
            onClick={() => setPaymentMethodPreset("last7days")}
            className={`py-1.5 px-1 rounded-md border text-center transition-all cursor-pointer shadow-2xs ${
              paymentMethodFilter.activePreset === "last7days"
                ? "bg-kv-blue-primary text-white border-kv-blue-primary font-black"
                : "bg-slate-50 text-slate-600 border-slate-200 hover:bg-kv-blue-light hover:text-kv-blue-primary"
            }`}
          >
          ...
          <button
            type="button"
            onClick={() => setPaymentMethodPreset("thisMonth")}
            className={`py-1.5 px-1 rounded-md border text-center transition-all cursor-pointer shadow-2xs ${
              paymentMethodFilter.activePreset === "thisMonth"
                ? "bg-kv-blue-primary text-white border-kv-blue-primary font-black"
                : "bg-slate-50 text-slate-600 border-slate-200 hover:bg-kv-blue-light hover:text-kv-blue-primary"
            }`}
          >
      ...
      <div className="flex flex-col gap-2 pt-2 border-t border-slate-100">
        ...
        <div className="flex flex-col gap-1.5">
          <div>
            ...
            <input
              type="date"
              value={paymentMethodFilter.fromDate}
              onChange={(e) =>
          ...
          <div>
            ...
            <input
              type="date"
              value={paymentMethodFilter.toDate}
              onChange={(e) =>
                setPaymentMethodFilter((prev) => ({
                  ...prev,
