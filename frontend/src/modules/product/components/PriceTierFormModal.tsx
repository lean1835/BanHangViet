    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs transition-opacity animate-in fade-in duration-200">
      ...
      >
          ...
          <div>
            ...
            {productName && (
              <p className="text-xs text-slate-500 mt-0.5 line-clamp-1">
                Mặt hàng: <strong className="text-slate-700">{productName}</strong>
              </p>
            )}
          </div>
          <button
        ...
        >
          ...
          {!isEditing && (
            <div>
              <span className="text-[11px] font-semibold text-slate-500 mb-1.5 flex items-center gap-1">
                ...
                <span>Gợi ý đặt tên bậc giá thông dụng:</span>
              </span>
              <div className="flex flex-wrap gap-1.5">
                {PRICE_TIER_COPY.QUICK_SUGGESTIONS.map((sug) => (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-sky-100 hover:text-sky-800 text-slate-600 text-[11px] font-medium border border-slate-200 transition-colors"
                  >
                    {sug}
                  </button>
                ))}
          ...
          {/* Tier Name */}
          <div>
            <label htmlFor="tierName" className="block font-bold text-slate-700 mb-1">
            ...
            <input
              ...
              className={`w-full px-3 py-2.5 rounded-xl border text-xs focus:ring-2 focus:ring-sky-500/20 outline-hidden transition-all ${
                errors.tierName
          ...
          {/* Unit selection & Current Retail Price info */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="unitConversionId" className="block font-bold text-slate-700 mb-1">
                {PRICE_TIER_COPY.UNIT_LABEL}
              </label>
              <select
                id="unitConversionId"
                {...register("unitConversionId")}
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-kv-blue-primary text-xs outline-hidden bg-white"
              >
                <option value="">
            ...
            <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex flex-col justify-center">
              ...
              <span className="font-extrabold text-slate-800 text-sm">
                {formatCurrency(currentRetailPrice)} / {currentUnitName}
              </span>
              {effectiveCost > 0 && (
                <span className="text-[10px] text-slate-400 mt-0.5">
                  Giá vốn đối chiếu: {formatCurrency(effectiveCost)}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="minQuantity" className="block font-bold text-slate-700 mb-1">
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-3 py-2.5 rounded-xl border text-xs focus:ring-2 focus:ring-sky-500/20 outline-hidden font-bold ${
                    errors.minQuantity
                      ? "border-rose-300 bg-rose-50/30 text-rose-900"
                      : "border-slate-200 focus:border-kv-blue-primary text-slate-800"
                  }`}
                />
                <span className="absolute right-3 top-2.5 text-slate-400 font-medium text-xs">
                  {currentUnitName}
                </span>
              </div>
              {errors.minQuantity && (
            ...
            <div>
              <label htmlFor="maxQuantity" className="block font-bold text-slate-700 mb-1">
          ...
          {/* Tier Price & Discount Percentage */}
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-500 font-medium">
                Giá lẻ niêm yết: <strong className="text-slate-700">{formatCurrency(currentRetailPrice)}/{currentUnitName}</strong>
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[10px] text-slate-400 font-medium">Tự tính đơn giá</span>
                </div>
                <div className="relative">
                  <input
                    id="discountPercent"
          ...
          <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
            <div>
              ...
              <span className="text-[11px] text-slate-400">
                Khi bật, hệ thống tại quầy POS sẽ tự động áp dụng bậc giá này khi đạt số lượng
              </span>
            </div>
        ...
        <div className="flex items-center justify-end gap-2.5 px-6 py-4 border-t border-slate-100 bg-slate-50/50">
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className="px-5 py-2 rounded-xl bg-kv-blue-primary hover:bg-kv-blue-dark active:scale-95 text-white font-bold text-xs shadow-xs transition-all flex items-center gap-1.5 disabled:opacity-50"
          >
            {isSubmitting ? (
              <>
