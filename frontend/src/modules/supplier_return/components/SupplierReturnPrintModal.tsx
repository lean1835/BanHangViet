...
import type { ISupplierReturnDetail } from "../types/ISupplierReturn";

interface SupplierReturnPrintModalProps {
  isOpen: boolean;
  onClose: () => void;
  returnDetail: ISupplierReturnDetail;
}
...
export const SupplierReturnPrintModal: React.FC<SupplierReturnPrintModalProps> = ({
  isOpen,
  onClose,
  returnDetail,
}) => {
  ...
  const totalAmount = returnDetail.totalReturnAmount ?? items.reduce(
    (sum, it) => sum + (Number(it.subtotal) || Number(it.quantity) * Number(it.purchasePrice)),
    0
  );
  ...
  return createPortal(
    <div
      id="supplier-return-print-modal-portal"
      role="dialog"
      aria-modal="true"
      aria-labelledby="supplier-return-print-modal-title"
    ...
    >
      <style>{`
        @media print {
          ...
          body > *:not(#supplier-return-print-modal-portal) {
            display: none !important;
          }
          #supplier-return-print-modal-portal {
          ...
          #supplier-return-print-scroll-container {
            position: static !important;
            display: block !important;
            width: 100% !important;
            margin: 0 !important;
      ...
      <div
        id="supplier-return-print-panel"
        ref={modalRef}
        tabIndex={-1}
      ...
      >
        ...
        <div className="bg-slate-800 text-white px-5 py-3 flex items-center justify-between shadow-sm shrink-0 no-print">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2
                id="supplier-return-print-modal-title"
                className="text-xs font-bold uppercase tracking-wider"
              >
                Mẫu Phiếu Xuất Trả Hàng Nhà Cung Cấp Chứng Từ
              </h2>
              <p className="text-[11px] text-slate-300 font-normal">
                Mã phiếu: <span className="font-mono text-white">{returnDetail.returnNumber}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            <button
              type="button"
              onClick={handlePrint}
              className="h-8 px-3.5 rounded-lg bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs flex items-center gap-1.5 transition-all shadow-sm active:scale-95"
            >
        ...
        <div
          id="supplier-return-print-scroll-container"
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 pb-1">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-black font-normal mt-0.5">
                  {household?.address || "123 Đường Giải Phóng, Quận Hai Bà Trưng, Hà Nội"}
                </p>
                <p className="text-[11px] text-black font-normal mt-0.5">
              ...
              <div className="text-left sm:text-right shrink-0 text-xs text-black space-y-1">
                ...
                <div className="flex sm:justify-end gap-2">
                  ...
                  <span className="font-normal text-black">
                    {formatDateDMY(returnDetail.returnDate || returnDetail.createdAt)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 text-xs text-black mb-4 pb-1 font-normal">
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-2">
                <div className="flex items-baseline flex-1 min-w-0">
                  ...
                  <span className="font-bold text-black border-b border-dotted border-black flex-1 min-w-0 truncate pb-0.5">
                    {supplier?.name || returnDetail.supplierName || "— (Nhập lẻ / Không chọn NCC)"}
                  </span>
                </div>
                <div className="flex items-baseline sm:w-64 shrink-0">
                  ...
                  <span className="font-normal text-black border-b border-dotted border-black flex-1 truncate pb-0.5">
                    {supplier?.phoneNumber || returnDetail.supplierPhone || "---"}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-2">
                <div className="flex items-baseline flex-1 min-w-0">
                  <span className="shrink-0 mr-2 text-black font-normal">Phiếu nhập gốc:</span>
                  <span className="font-semibold text-black font-mono border-b border-dotted border-black flex-1 min-w-0 truncate pb-0.5">
                    {returnDetail.receiptNumber || returnDetail.receiptId || "---"}
                  </span>
                </div>
                <div className="flex items-baseline sm:w-64 shrink-0">
                  <span className="shrink-0 mr-2 text-black font-normal">Lý do trả:</span>
                  <span className="font-normal text-black border-b border-dotted border-black flex-1 truncate pb-0.5">
              ...
              <table className="w-full text-left border-collapse border border-black text-[11px] sm:text-xs">
                <thead>
                  <tr className="bg-neutral-200 text-black font-bold border-b border-black text-center">
                    ...
                    <th className="p-1.5 border-r border-black w-24 text-center">Đơn giá nhập</th>
                ...
                <tbody className="text-black font-normal">
                        ...
                        <td className="p-1.5 border-r border-black text-left">
                          ...
                          {hasConversion && (
                            <div className="text-[10px] text-neutral-600 italic">
                              Quy đổi: 1 {item.unitName} = {item.conversionFactor} đơn vị chuẩn
                            </div>
                          )}
                        </td>
                  ...
                  <tr className="border-b border-black text-black">
                    ...
                    <td className="p-1.5 border-r border-black text-right font-bold">
                      {formatCurrency(returnDetail.supplierDebtReduced ?? totalAmount)}
                    </td>
            ...
            <div className="mt-6">
              <div className="flex justify-end text-xs text-black italic mb-2">
                <span>{formatFullDateVietnamese(returnDetail.returnDate || returnDetail.createdAt)}</span>
              </div>
...
export default SupplierReturnPrintModal;
