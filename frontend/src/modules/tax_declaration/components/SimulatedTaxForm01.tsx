...
export const SimulatedTaxForm01: React.FC<ISimulatedTaxForm01Props> = ({
  ...
  return (
    <div
      id={id}
      className="bg-white p-6 sm:p-8 md:p-12 text-slate-900 font-sans border border-slate-300 shadow-sm max-w-4xl mx-auto rounded-2xl print:shadow-none print:border-none print:p-0 print:m-0 print:max-w-none print:w-full print:rounded-none min-h-[1050px] print:min-h-0"
    >
      {/* 1. Header Quốc hiệu & Tiêu ngữ */}
      <div className="flex flex-col sm:flex-row justify-between items-start border-b border-slate-300 pb-3.5 print:pb-2 text-xs font-semibold">
      ...
      {/* 2. Tiêu đề Tờ khai */}
      <div className="text-center my-5 print:my-2.5">
        ...
        <div className="flex items-center justify-center gap-6 mt-2.5 print:mt-1.5 text-xs font-bold text-slate-700">
          <span>
      ...
      <div className="border border-slate-300 rounded-xl p-3.5 print:p-2.5 bg-slate-50/50 mb-5 print:mb-2.5 text-xs space-y-2 print:space-y-1">
      ...
      <div className="mb-5 print:mb-2.5">
        <h3 className="text-xs font-black uppercase text-slate-800 mb-2 print:mb-1">
        ...
        <div className="overflow-x-auto border border-slate-300 rounded-xl">
          <table className="w-full text-xs text-left border-collapse">
            <thead className="bg-slate-100 border-b border-slate-300 text-slate-700 font-bold uppercase text-[10px]">
              <tr>
                <th className="p-2 print:p-1.5 border-r border-slate-300 text-center w-10">STT</th>
                <th className="p-2 print:p-1.5 border-r border-slate-300">Nhóm ngành nghề / Thuế suất</th>
                <th className="p-2 print:p-1.5 border-r border-slate-300 text-right">Doanh thu chịu thuế</th>
                <th className="p-2 print:p-1.5 border-r border-slate-300 text-center w-16">Thuế GTGT</th>
                <th className="p-2 print:p-1.5 border-r border-slate-300 text-right">Tiền thuế GTGT (1%)</th>
                <th className="p-2 print:p-1.5 border-r border-slate-300 text-center w-16">Thuế TNCN</th>
                <th className="p-2 print:p-1.5 border-r border-slate-300 text-right">Tiền thuế TNCN (0.5%)</th>
                <th className="p-2 print:p-1.5 text-right">Tổng thuế phải nộp</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              {taxRateSummaries.length > 0 ? (
                taxRateSummaries.map((group, idx) => {
                  ...
                  return (
                    <tr key={idx} className="hover:bg-slate-50">
                      <td className="p-2 print:p-1.5 border-r border-slate-300 text-center font-bold text-slate-600">
                        {idx + 1}
                      </td>
                      <td className="p-2 print:p-1.5 border-r border-slate-300 font-medium">
                        {group.taxRateName || `Hàng hóa chịu thuế suất ${group.taxRatePercentage}%`}
                      </td>
                      <td className="p-2 print:p-1.5 border-r border-slate-300 text-right font-semibold text-slate-800">
                        {formatCurrency(group.revenueAmount)}
                      </td>
                      <td className="p-2 print:p-1.5 border-r border-slate-300 text-center font-bold text-slate-600">
                        1%
                      </td>
                      <td className="p-2 print:p-1.5 border-r border-slate-300 text-right font-semibold text-emerald-700">
                        {formatCurrency(groupVat)}
                      </td>
                      <td className="p-2 print:p-1.5 border-r border-slate-300 text-center font-bold text-slate-600">
                        0.5%
                      </td>
                      <td className="p-2 print:p-1.5 border-r border-slate-300 text-right font-semibold text-indigo-700">
                        {formatCurrency(groupPit)}
                      </td>
                      <td className="p-2 print:p-1.5 text-right font-extrabold text-rose-700">
                ...
                <tr>
                  <td className="p-2 print:p-1.5 border-r border-slate-300 text-center font-bold text-slate-600">1</td>
                  <td className="p-2 print:p-1.5 border-r border-slate-300 font-medium">Phân phối, cung cấp hàng hóa (Thuế suất 8%)</td>
                  <td className="p-2 print:p-1.5 border-r border-slate-300 text-right font-semibold text-slate-800">{formatCurrency(totalRevenue)}</td>
                  <td className="p-2 print:p-1.5 border-r border-slate-300 text-center font-bold text-slate-600">1%</td>
                  <td className="p-2 print:p-1.5 border-r border-slate-300 text-right font-semibold text-emerald-700">{formatCurrency(vatAmountEstimated)}</td>
                  <td className="p-2 print:p-1.5 border-r border-slate-300 text-center font-bold text-slate-600">0.5%</td>
                  <td className="p-2 print:p-1.5 border-r border-slate-300 text-right font-semibold text-indigo-700">{formatCurrency(pitAmountEstimated)}</td>
                  <td className="p-2 print:p-1.5 text-right font-extrabold text-rose-700">{formatCurrency(totalTaxAmount)}</td>
                </tr>
              )}
              {/* Dòng Tổng cộng */}
              <tr className="bg-slate-100 font-extrabold text-slate-900">
                <td colSpan={2} className="p-2 print:p-1.5 border-r border-slate-300 text-center uppercase">
                  TỔNG CỘNG NGHĨA VỤ THUẾ CỦA KỲ
                </td>
                <td className="p-2 print:p-1.5 border-r border-slate-300 text-right text-blue-700">
                  {formatCurrency(totalRevenue)}
                </td>
                <td className="p-2 print:p-1.5 border-r border-slate-300 text-center">-</td>
                <td className="p-2 print:p-1.5 border-r border-slate-300 text-right text-emerald-700">
                  {formatCurrency(vatAmountEstimated)}
                </td>
                <td className="p-2 print:p-1.5 border-r border-slate-300 text-center">-</td>
                <td className="p-2 print:p-1.5 border-r border-slate-300 text-right text-indigo-700">
                  {formatCurrency(pitAmountEstimated)}
                </td>
                <td className="p-2 print:p-1.5 text-right text-rose-700 text-sm">
      ...
      <div className="text-xs space-y-1.5 print:space-y-1 mb-5 print:mb-2.5 border-t border-slate-200 pt-3 print:pt-1.5">
        <p>
      ...
      <div className="grid grid-cols-2 gap-6 print:gap-4 text-center text-xs mt-4 print:mt-2">
        <div>
          ...
          <div className="h-16 print:h-12 flex items-center justify-center text-slate-400 italic text-[11px]">
        ...
        <div>
          ...
          <p className="font-bold uppercase text-slate-800 mt-0.5">
            NGƯỜI NỘP THUẾ hoặc ĐẠI DIỆN HỢP PHÁP
          </p>
          <p className="text-[11px] text-slate-400 italic mt-0.5">(Ký, đóng dấu hoặc ký số điện tử)</p>
          <div className="h-16 print:h-12 flex flex-col items-center justify-center">
            <div className="border border-emerald-500 bg-emerald-50 px-2.5 py-0.5 rounded text-emerald-800 font-extrabold text-[10px] uppercase">
              ✓ Ký điện tử: {householdData?.name || "Hộ kinh doanh Bán Hàng Việt"}
            </div>
            <span className="font-bold text-slate-800 mt-0.5 text-xs">
      ...
      <div className="text-center mt-5 print:mt-2 pt-3 print:pt-1.5 border-t border-dashed border-slate-200 text-[10px] text-slate-400">
