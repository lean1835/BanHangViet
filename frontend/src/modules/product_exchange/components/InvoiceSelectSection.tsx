...
export const checkIsInvoiceExchangedOrAdjusted = (inv?: IInvoice | null): boolean => {
  if (!inv) return false;
  return Boolean(
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-5">
      ...
      {!selectedInvoice ? (
          ...
          <div className="max-h-64 overflow-y-auto space-y-2 pr-1">
            {isLoading ? (
              ...
              invoices.map((inv) => {
                ...
                const isExchangedOrAdjusted = checkIsInvoiceExchangedOrAdjusted(inv);
                const isClickable = !isOverdue && !isExchangedOrAdjusted;

                return (
                  <div
                    key={inv.id}
                    onClick={() => isClickable && onSelectInvoice(inv)}
                    className={`p-3.5 rounded-lg border transition-all flex flex-col md:flex-row md:items-center justify-between gap-3 ${
                      !isClickable
                        ? "border-rose-200 bg-rose-50/30 dark:border-rose-900/30 dark:bg-rose-950/20 opacity-60 cursor-not-allowed"
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-3 self-end md:self-center">
                      ...
                      {isExchangedOrAdjusted ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-semibold bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
                          <AlertTriangle className="w-3 h-3" /> Đã đổi/trả (Không được đổi lại)
                        </span>
                      ) : isOverdue ? (
        ...
        const isSelectedIneligible =
          checkIsInvoiceExchangedOrAdjusted(selectedInvoice) ||
          (eligibilityData && eligibilityData.isEligible === false);
        const ineligibilityReason =
          eligibilityData?.message ||
          (checkIsInvoiceExchangedOrAdjusted(selectedInvoice)
            ? "Hóa đơn này đã từng đổi/trả hoặc là hóa đơn bổ sung, không được phép đổi trả lại theo quy định."
            : "Hóa đơn không đủ điều kiện đổi hàng.");

        return (
          <div
            className={`p-4 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4 border ${
          ...
          >
            <div className="flex items-start gap-3.5">
              ...
              >
                {isSelectedIneligible ? <AlertTriangle className="w-5 h-5" /> : <CheckCircle2 className="w-5 h-5" />}
              </div>
              <div>
                ...
                {isSelectedIneligible && (
                  <p className="text-xs font-medium text-rose-700 dark:text-rose-300 mt-1">
                    {ineligibilityReason}
                  </p>
                )}
            ...
            <div
              className={`text-right border-t md:border-t-0 pt-2 md:pt-0 ${
                isSelectedIneligible
